 ...
 * <P>
 * Description: Wraps a stream and provides helper methods for the construction
 * of records.
 * <P>
 * 
...
public final class RecordInputStream implements LittleEndianInput {
	 ...
	 * previous BIFF record is finished, the next sid has been properly read,
	 * but the data size field has not been read yet.
	 */
	 ...
	 * don't completely read all available data. This exception should never be
	 * thrown otherwise.
	 */
	...
	/**
	 * Length of the data section of the current BIFF record (always 4 less than
	 * the total record size). When uninitialised, this field is set to
	 * {@link #DATA_LEN_NEEDS_TO_BE_READ}.
	 */
	private int _currentDataLength;
	/**
	 * The BIFF record identifier for the next record is read when just as the
	 * current record is finished. This field is only really valid during the
	 * time that ({@link #_currentDataLength} ==
	 * {@link #DATA_LEN_NEEDS_TO_BE_READ}). At most other times its value is not
	 * really the 'sid of the next record'. Wwhile mid-record, this field
	 * coincidentally holds the sid of the current record.
	 */
	...
	static LittleEndianInput getLEI(InputStream is) {
		...
		// less optimal, but should work OK just the same. Often occurs in junit
		// tests.
	...
	/**
	 * Note - this method is expected to be called only when completed reading
	 * the current BIFF record.
	 * 
	 * @throws LeftoverDataException
	...
	/**
	 * @return the sid of the next record or {@link #INVALID_SID_VALUE} if at
	 *         end of stream
	 */
	 ...
	 * 
	 * @deprecated Best to write a input stream that wraps this one where there
	 *             is special sub record that may overlap continue records.
	 */
	...
	private boolean isContinueNext() {
		...
		// - Often from within the char data of long strings (caller is within
		// readStringCommon()).
		// - From UnicodeString construction (many different points - call via
		// checkRecordPosition)
		// - During TextObjectRecord construction (just before the text, perhaps
		// within the text,
